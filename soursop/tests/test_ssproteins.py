...
def test_contacts_special(NTL9_CP):
    ...
    with pytest.raises(SSException) as error:
        a = NTL9_CP.get_contact_map(
            distance_thresh=2, stride=1, mode="sidechain-invalid"
        )
...
def test_code_coverage(NTL9_CP):
    ...
    a = NTL9_CP.calculate_all_CA_distances(
        10, stride=2, mode="COM", only_C_terminal_residues=False
    )
    ...
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="COM", RMS=True, return_instantaneous_maps=True
    )
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="COM", RMS=True, stride=2, return_instantaneous_maps=True
    )
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="CA", RMS=True, return_instantaneous_maps=True
    )
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="CA", RMS=True, stride=2, return_instantaneous_maps=True
    )
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="COM", RMS=True, return_instantaneous_maps=True
    )
    a = NTL9_CP.get_distance_map(
        verbose=False, mode="COM", return_instantaneous_maps=True
    )
    ...
    a = NTL9_CP.get_polymer_scaled_distance_map(
        nu=0.54, A0=6, min_separation=20, mode="signed-fractional-change"
    )
    a = NTL9_CP.get_polymer_scaled_distance_map(
        nu=0.54, A0=6, min_separation=20, mode="signed-absolute-change"
    )
    a = NTL9_CP.get_polymer_scaled_distance_map(
        nu=0.54, A0=6, min_separation=20, mode="scaled"
    )
...
def test_get_radius_of_gyration(GS6_CP):
    ...
    assert (
        abs(GS6_CP.get_radius_of_gyration(R1=1, R2=3)[0] - 2.8815625777553278) < 0.0001
    )
    assert (
        GS6_CP.get_radius_of_gyration(R1=0, R2=7)[0]
        == GS6_CP.get_radius_of_gyration()[0]
    )
...
def test_get_internal_scaling(GS6_CP):
    ...
    assert (
        abs(np.mean(GS6_CP.get_internal_scaling()[1][1]) - 3.784701967415692) < 0.0001
    )
    assert (
        abs(np.mean(GS6_CP.get_internal_scaling()[1][2]) - 6.524909331438218) < 0.0001
    )
    assert (
...
def test_repr(GS6_CP, NTL9_CP):
    ...
    for protein in proteins:
        ...
        rebuilt_repr = "SSProtein (%s): %i res and %i frames" % (
            hex(id(protein)),
            protein.n_residues,
            protein.n_frames,
        )
...
def test_check_weights_valid_uniform_weights_length_large_etol(GS6_CP, NTL9_CP):
    ...
    for protein in proteins:
        weights = [default_weight for frame in range(protein.n_frames)]
        normalized_weights = [w / protein.n_frames for w in weights]
        out = protein._SSProtein__check_weights(
            weights=normalized_weights, etol=tolerance
        )
...
def test_get_selection_atoms_invalid_region_of_size_3(GS6_CP, NTL9_CP):
    ...
    for protein in proteins:
        region = (
            0,
            protein.n_residues - 1,
            0,
        )  # the last index doesn't matter (will raise exception regardless)
        choices = itertools.product([True, False], repeat=2)
        for backbone, heavy in choices:
            with pytest.raises(SSException):
                protein._SSProtein__get_selection_atoms(region, backbone, heavy)
    ...
    for protein in proteins:
        ...
        for mode in modes:
            for rms_option in rms_options:
                results = protein.get_distance_map(
                    mode=mode, RMS=rms_option, weights=normalized_weights
                )
    ...
    for protein in proteins:
        ...
        for mode in modes:
            if mode == "sidechain-heavy":
                with pytest.raises(SSException):
                    results = protein.get_contact_map(
                        mode=mode, weights=normalized_weights
                    )  # the other options have been tested
                continue
            results = protein.get_contact_map(
                mode=mode, weights=normalized_weights
            )  # the other options have been tested
...
def test_get_sidechain_alignment_angle_unsupported_angle(GS6_CP, NTL9_CP):
    ...
    for protein in proteins:
        with pytest.raises(SSException):
            protein.get_sidechain_alignment_angle(
                R1, R2, sidechain_atom_1="invalid_atom"
            )

        with pytest.raises(SSException):
            protein.get_sidechain_alignment_angle(
                R1, R2, sidechain_atom_2="invalid_atom"
            )
...
def test_get_sidechain_alignment_angle_successful_random(GS6_CP, NTL9_CP):
    ...
    for protein in proteins:
        ...
        for residue_name in error_residues:
            error_residue_indices += list(
                sorted(
                    set([i - 1 for i in df[df["resName"] == residue_name]["resSeq"]])
                )
            )
...
def test_get_local_to_global_correlation_valid_weights(
    GS6_CP, NTL9_CP, cta_protein_helper
):
    num_copies = 5
    stride = 1
    default_weight = 1.0
    ...
    modes = "CA,COM".split(",")
    for protein_obj in proteins:
        protein = cta_protein_helper.lengthen_protein_trajectory(
            protein_obj, num_copies
        )
        weights = [default_weight for frame in range(protein.n_frames)]
        normalized_weights = [w / protein.n_frames for w in weights]
        for mode in modes:
            results = protein.get_local_to_global_correlation(
                mode=mode, stride=stride, weights=normalized_weights
            )
...
def test_get_all_SASA(GS6_CP, NTL9_CP):
    ...
    assert np.isclose(
        np.mean(GS6_CP.get_all_SASA(stride=1, mode="residue")), 108.992676
    )
    ...
    assert (
        np.sum(
            GS6_CP.get_all_SASA(stride=1, mode="all")[1]
            == GS6_CP.get_all_SASA(stride=1, mode="sidechain")
        )
        == 30
    )
    assert (
        np.sum(
            GS6_CP.get_all_SASA(stride=1, mode="all")[2]
            == GS6_CP.get_all_SASA(stride=1, mode="backbone")
        )
        == 30
    )
...
def test_get_inter_residue_COM_vector(GS6_CP, NTL9_CP):
    ...
    com_distances_manually = np.sqrt(
        np.sum(np.square(GS6_CP.get_inter_residue_COM_vector(0, 5)), 1)
    )
    assert np.sum(com_distances - com_distances_manually) < 0.001

    com_distances = NTL9_CP.get_inter_residue_COM_distance(0, 25)
    com_distances_manually = np.sqrt(
        np.sum(np.square(NTL9_CP.get_inter_residue_COM_vector(0, 25)), 1)
    )
    assert np.sum(com_distances - com_distances_manually) < 0.001
