...
def test_hellingers_distance():
    ...
    p = np.array([[1, 0], [1, 0]])
    q = np.array([[0, 1], [0, 1]])
    assert np.all(hellinger_distance(p, q) == np.array([1.0, 1.0]))

    p = np.array([[0, 0], [1, 0]])
    q = np.array([[0, 0], [0, 1]])
    assert np.all(hellinger_distance(p, q) == np.array([0.0, 1.0]))
...
def test_sampling_quality():
    ...
    hellingers = np.array(
        [
            [
                [0.24167835, 0.19611402],
                [0.24688875, 0.24731097],
                [0.24903448, 0.20723213],
            ],
            [[0.13862987, 0.1848602], [0.159583, 0.18751601], [0.1429838, 0.13090839]],
        ]
    )
    ...
    test_pdf = np.apply_along_axis(
        lambda col: np.histogram(col, bins=bins, density=True)[0], axis=1, arr=test_psi
    ) * np.round(np.rad2deg(alanine_qual.bwidth))

    test_ref_pdf = np.apply_along_axis(
    ...
    assert np.allclose(
        test_pdf, alanine_qual.compute_pdf(alanine_qual.psi_angles[0], bins=bins)
    )

    assert np.allclose(
        test_ref_pdf,
        alanine_qual.compute_pdf(alanine_qual.ref_psi_angles[0], bins=bins),
    )
...
def test_hellingers_2d():
    ...
    hellingers_2d = np.array(
        [[0.44302328, 0.42337237], [0.46023975, 0.46853325], [0.42439771, 0.42886934]]
    )
