...
1. **Sequence-based random-coil chemical shifts** — predicts random-coil
   ...
   sequence, corrected for temperature, pH and (optionally) perdeuteration,
   including phosphorylated Ser/Thr/Tyr. Implementation ports the
   Kjaergaard & Poulsen / Schwarzinger reference-shift and neighbour-
   correction tables. The public entry point is
   :func:`compute_random_coil_chemical_shifts`.

2. **Structure-based scalar (J) couplings** — predicts the backbone
   ³J(HN, Hα) scalar coupling per residue per frame from the φ dihedral
   via the Karplus relation, using any of the six literature
   parameterisations stored in :data:`KARPLUS_HN_HA_COEFFICIENTS`
   (Bax2007, Bax1997, Ruterjans1999, Habeck, Vuister, Pardi). Public
   entry points are :func:`karplus` (generic Karplus evaluator for
   arbitrary coefficients) and :func:`compute_J3_HN_HA` (operates on an
   :class:`~soursop.ssprotein.SSProtein`). The returned ``(n_frames,
...
**Author(s):** Alex Keeley (chemical shifts) and Alex Holehouse
(J-couplings).
"""
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
def compute_random_coil_chemical_shifts(
    protein_sequence,
    temperature=25,
    pH=7.4,
    use_ggxgg=True,
    ...
    gly_ca_c[key_aa3["PSER"]] = gly_ca_c[key_aa3["SER"]]
    gly_ca_c[key_aa3["PTHR"]] = gly_ca_c[key_aa3["THR"]]
    gly_ca_c[key_aa3["PTYR"]] = gly_ca_c[key_aa3["TYR"]]
    ...
    gly_co_c[key_aa3["PSER"]] = gly_co_c[key_aa3["SER"]]
    gly_co_c[key_aa3["PTHR"]] = gly_co_c[key_aa3["THR"]]
    gly_co_c[key_aa3["PTYR"]] = gly_co_c[key_aa3["TYR"]]
    ...
    gly_n_c[key_aa3["PSER"]] = gly_n_c[key_aa3["SER"]]
    gly_n_c[key_aa3["PTHR"]] = gly_n_c[key_aa3["THR"]]
    gly_n_c[key_aa3["PTYR"]] = gly_n_c[key_aa3["TYR"]]
    ...
    asp_ph_0 = [
        [53.05, 37.81, 175.25, 120.10, 8.68, 4.70],
        [54.59, 40.95, 176.53, 121.78, 8.57, 4.58],
    ]

    glu_ph_0 = [
        [55.90, 28.65, 176.19, 122.04, 8.58, 4.36],
        [56.81, 30.19, 176.71, 122.86, 8.69, 4.26],
    ]

    his_ph_0 = [
        [55.30, 28.89, 174.37, 120.00, 8.78, 4.69],
        [56.69, 31.27, 175.85, 121.46, 8.44, 4.63],
    ]

    sep_ph_0 = [
        [57.613, 66.527, 174.062, 116.940, 8.805, 4.525],
        [58.433, 65.463, 174.977, 119.488, 9.242, 4.409],
    ]

    tpo_ph_0 = [
        [61.827, 73.986, 174.063, 115.936, 8.615, 4.407],
        [63.524, 71.718, 174.793, 121.101, 9.397, 4.167],
    ]

    ptr_ph_0 = [
        [57.905, 38.760, 175.605, 121.797, 8.403, 4.604],
    ...
    delta_T = (
        temperature - 5
    )  # difference between the given temperature and 5 degrees C
    ca_pred = 0
    cb_pred = 0
    co_pred = 0
    n_pred = 0
    ...
    for i in range(6):
        asp_ph_corr[i] = (
            asp_deprot_frac * asp_ph_0[1][i] + (1 - asp_deprot_frac) * asp_ph_0[0][i]
        )
        glu_ph_corr[i] = (
            glu_deprot_frac * glu_ph_0[1][i] + (1 - glu_deprot_frac) * glu_ph_0[0][i]
        )
        his_ph_corr[i] = (
            his_deprot_frac * his_ph_0[1][i] + (1 - his_deprot_frac) * his_ph_0[0][i]
        )
        sep_ph_corr[i] = (
            sep_deprot_frac * sep_ph_0[1][i] + (1 - sep_deprot_frac) * sep_ph_0[0][i]
        )
        tpo_ph_corr[i] = (
            tpo_deprot_frac * tpo_ph_0[1][i] + (1 - tpo_deprot_frac) * tpo_ph_0[0][i]
        )
        ptr_ph_corr[i] = (
            ptr_deprot_frac * ptr_ph_0[1][i] + (1 - ptr_deprot_frac) * ptr_ph_0[0][i]
        )
    ...
    while cur < (len(sequence) - 4):
        ...
        if sequence[cur + 2] == 5 and use_ggxgg:  # special case of glycine
            ca_pred += (
                gly_ca_a[sequence[cur + 4]]
                + gly_ca_b[sequence[cur + 3]]
                + gly_ca_c[sequence[cur + 1]]
                + gly_ca_d[sequence[cur]]
                + (delta_T * ca_t[sequence[cur + 2]] / 1000)
            )
            co_pred += (
                gly_co_a[sequence[cur + 4]]
                + gly_co_b[sequence[cur + 3]]
                + gly_co_c[sequence[cur + 1]]
                + gly_co_d[sequence[cur]]
                + (delta_T * co_t[sequence[cur + 2]] / 1000)
            )
            n_pred += (
                gly_n_a[sequence[cur + 4]]
                + gly_n_b[sequence[cur + 3]]
                + gly_n_c[sequence[cur + 1]]
                + gly_n_d[sequence[cur]]
                + (delta_T * n_t[sequence[cur + 2]] / 1000)
            )
        ...
        cb_pred += (
            ...
            + cb_c[sequence[cur + 1]]
            + cb_d[sequence[cur]]
            + (delta_T * cb_t[sequence[cur + 2]] / 1000)
        )
        hn_pred += (
            hn_a[sequence[cur + 4]]
            + hn_b[sequence[cur + 3]]
            + hn_c[sequence[cur + 1]]
            + hn_d[sequence[cur]]
            + (delta_T * hn_t[sequence[cur + 2]] / 1000)
        )
        ha_pred += (
            ha_a[sequence[cur + 4]]
            + ha_b[sequence[cur + 3]]
            + ha_c[sequence[cur + 1]]
            + ha_d[sequence[cur]]
            + (delta_T * ha_t[sequence[cur + 2]] / 1000)
        )
...
# Scalar (J) couplings
#
# Karplus coefficients for the backbone 3J(HN, H_alpha) scalar coupling
# expressed as a function of the phi dihedral. Six literature
# parameterisations are exposed; values are ported verbatim from biceps'
# ``J_coupling.py`` (Voelz lab, github.com/vvoelz/biceps), itself adapted
# from MDTraj's ``mdtraj/nmr/scalar_couplings.py`` (Beauchamp et al.).
# ``phi0`` is stored here in DEGREES (biceps stores it in radians) for
# consistency with the rest of soursop's dihedral handling, which uses
# degrees throughout.
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------

#: Karplus coefficients for 3J(HN, H_alpha) — ``J = A cos^2(phi + phi0) +
#: B cos(phi + phi0) + C`` (angles in degrees, J in Hz). Six literature
#: parameterisations. See module docstring for citations.
KARPLUS_HN_HA_COEFFICIENTS = {
    "Ruterjans1999": dict(phi0=-60.0, A=7.90, B=-1.05, C=0.65),
    "Bax2007": dict(phi0=-60.0, A=8.40, B=-1.36, C=0.33),
    "Bax1997": dict(phi0=-60.0, A=7.09, B=-1.42, C=1.55),
    "Habeck": dict(phi0=-60.0, A=7.13, B=-1.31, C=1.56),
    "Vuister": dict(phi0=-60.0, A=6.51, B=-1.76, C=1.60),
    "Pardi": dict(phi0=-60.0, A=6.40, B=-1.40, C=1.90),
}

#: Per-model RMSD between Karplus prediction and experiment, in Hz
#: (treat as a forward-model uncertainty when feeding J-couplings into
#: BME/COPER).
KARPLUS_HN_HA_UNCERTAINTIES = {
    "Ruterjans1999": 0.25,
    "Bax2007": 0.36,
    "Bax1997": 0.39,
    "Habeck": 0.34,
...
#: Default Karplus parameterisation used by :func:`compute_J3_HN_HA`.
KARPLUS_HN_HA_DEFAULT_MODEL = "Bax2007"


# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
def karplus(angle, A, B, C, phi0=0.0):
    """Generic Karplus relation ``J = A cos^2(theta) + B cos(theta) + C``.

    ``theta = angle + phi0`` with both quantities in **degrees**; the
    returned scalar coupling is in **Hz**. Vectorised — accepts a scalar,
    1D array or higher-dimensional array of ``angle``.

    The shape of the input ``angle`` is preserved in the output, so this
    function evaluates the Karplus form for any literature
    parameterisation (the protein ³J(HN,Hα) sets in
    :data:`KARPLUS_HN_HA_COEFFICIENTS`, but also Bothner-By, Tvaroska,
    Aydin or any other Karplus-type relation with its own ``A``, ``B``,
    ``C`` and ``phi0``).

    Parameters
    ----------
    angle : float or numpy.ndarray
        Dihedral angle(s) in degrees.
    A, B, C : float
        Karplus coefficients.
    phi0 : float, optional
        Phase offset (degrees) added to ``angle`` before evaluating the
        Karplus form. Default ``0.0``.

    Returns
    -------
    float or numpy.ndarray
        ``A cos^2(angle + phi0) + B cos(angle + phi0) + C``, same shape
        as ``angle``, in Hz.
    ...
    """
    theta = np.deg2rad(np.asarray(angle, dtype=np.float64) + phi0)
    c = np.cos(theta)
    return A * c * c + B * c + C


# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
def compute_J3_HN_HA(
    protein,
    model=KARPLUS_HN_HA_DEFAULT_MODEL,
    stride=1,
    weights=False,
    etol=1e-7,
    ...
    Evaluates the Karplus relation on the per-frame φ dihedral angles
    (in degrees) returned by
    :meth:`~soursop.ssprotein.SSProtein.get_angles` using the chosen
    ...
    protein : soursop.ssprotein.SSProtein
        Protein chain whose ensemble we want J-couplings for.
    model : str, optional
        Karplus parameterisation. Must be a key of
        :data:`KARPLUS_HN_HA_COEFFICIENTS`. Default
        :data:`KARPLUS_HN_HA_DEFAULT_MODEL` (``"Bax2007"``).
    stride : int, optional
        Subsample the trajectory by taking every ``stride``-th frame
        before evaluation. Default ``1``.
    weights : numpy.ndarray or False, optional
        Optional per-frame weight vector (length ``n_frames``) used to
        collapse the frame axis to a single per-residue ensemble mean,
        validated by :func:`soursop.ssutils.validate_weights` (so the
        usual ``[0, 1]``, finite, ``sum(w) == 1`` contract applies). When
        ``stride`` and ``weights`` are both given the weight vector is
        first subsampled and re-normalised, matching the consistent
        package-wide reweighting behaviour. Default ``False`` (no
        weighted collapse — the full ``(n_frames, n_phi)`` matrix is
        returned).
    etol : float, optional
        Tolerance for the ``sum(weights) == 1`` check. Default ``1e-7``.
    return_uncertainty : bool, optional
        If True, additionally return the Karplus-model RMSD-vs-experiment
        from :data:`KARPLUS_HN_HA_UNCERTAINTIES` (scalar, Hz). This is a
        useful default forward-model uncertainty for use with BME/COPER.
        Default ``False``.

    Returns
    -------
    atom_indices : list of list of mdtraj.Atom
        The four atoms (C_{i-1}, N_i, CA_i, C_i) defining each φ
        dihedral. ``len(atom_indices) == n_phi``.
    J : numpy.ndarray
        ³J(HN, Hα) in Hz. Shape ``(n_frames, n_phi)`` by default; shape
        ``(n_phi,)`` when ``weights`` is supplied (frame axis collapsed
        to the weighted mean).
    sigma : float
        Karplus-model uncertainty in Hz. Returned only if
        ``return_uncertainty=True``.

    Raises
    ------
    SSException
        If ``model`` is not a key of :data:`KARPLUS_HN_HA_COEFFICIENTS`,
        or if ``weights`` fails :func:`soursop.ssutils.validate_weights`.
    ...
    """
    validate_keyword_option(model, list(KARPLUS_HN_HA_COEFFICIENTS.keys()), "model")
    coefficients = KARPLUS_HN_HA_COEFFICIENTS[model]

    # protein.get_angles('phi') returns [atom_lists, angles] with angles
    # in degrees and shape (n_phi, n_frames). Transpose to (n_frames, n_phi)
    # to match the convention used by BME / COPER / mdtraj.
    atom_indices, phi_per_residue = protein.get_angles("phi")
    phi = np.asarray(phi_per_residue).T  # (n_frames, n_phi)

    # Stride and weight handling follow the package-wide reweighting
    ...
    J = karplus(phi_strided, **coefficients)  # (n_frames_eff, n_phi)

    if validated_weights is not False:
        J = weighted_mean(J, validated_weights, axis=0)

    if return_uncertainty:
        return atom_indices, J, float(KARPLUS_HN_HA_UNCERTAINTIES[model])
    return atom_indices, J


# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
# NOE-derived distances
#
# Nuclear Overhauser Effect (NOE) cross-peak volumes are proportional to
# <r^-6> (or sometimes <r^-3>) averaged over the ensemble, so the linear
# observable for reweighting against an experimental NOE distance r_exp
# is r^-p, not r itself. We expose two helpers: ``compute_NOE_distances``
# returns per-frame inter-atom distances for an arbitrary list of atom
# pairs, and ``noe_ensemble_average`` collapses a per-frame distance
# array via the NOE convention (with optional reweighting).
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------

#: Default NOE averaging exponent (r^-6 convention).
DEFAULT_NOE_POWER = 6


def compute_NOE_distances(protein, atom_pairs, stride=1):
    ...
    against an experimental ``r_exp**-p``).

    Parameters
    ----------
    protein : soursop.ssprotein.SSProtein
    atom_pairs : array_like, shape (n_pairs, 2)
        Zero-based atom indices into ``protein.traj.topology``. (Use
        ``protein.traj.topology.select(...)`` or
        ``topology.atom(...)`` to translate residue/atom names into
        indices.)
    stride : int, optional
        Subsample frames before the distance computation. Default 1.

    Returns
    -------
    numpy.ndarray
        Distances in Angstroms, shape ``(n_frames, n_pairs)``
        (``n_frames`` after striding).
...
def noe_ensemble_average(
    ...
    Implements :math:`\\big( \\sum_i w_i\\, d_i^{-p} \\big)^{-1/p}` along
    ``axis``: the standard NOE r^-p ensemble convention (default
    ``p=6``; some studies use ``p=3``). Honours the package-wide
    weights= contract (see :func:`soursop.ssutils.validate_weights`):
    ``weights=False`` (default) gives the uniform mean, while a vector
    of per-frame weights (must be length ``distances.shape[axis]``,
    in [0, 1], finite, summing to 1) gives the weighted NOE distance.

    Parameters
    ----------
    distances : numpy.ndarray
        Distance array (any shape) — typically ``(n_frames, n_pairs)``
        from :func:`compute_NOE_distances`.
    power : float, optional
        NOE exponent. Default 6.
    weights : numpy.ndarray or False, optional
        Per-frame weights, validated by
    ...
    if np.any(d <= 0):
        raise SSException(
            ...
            f"min {d.min()}); r^-p is undefined at r <= 0."
        )
    ...
    if validated is False:
        # uniform mean
        mean_inv_p = inv_p.mean(axis=axis)
    else:
        mean_inv_p = weighted_mean(inv_p, validated, axis=axis)

    return mean_inv_p ** (-1.0 / float(power))
