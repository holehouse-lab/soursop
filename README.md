...
* `ssprotein.get_Q`: `stride != 1` and `weights` can now be used together (previously hard-blocked). The weight contract is now consistent with the rest of the package - one entry per trajectory frame (`len == n_frames`), validated by `ssutils.validate_weights`; for the `protein_average=False` breakdown the vector is strided, the native reference frame is excluded, and the remainder renormalised so the weighted average is a proper expectation over the non-native (strided) frames. `protein_average=True` still (intentionally) refuses weights.
